...
// IMPORTANT: Associate Boletim with Disciplina for fetching discipline name
...
// Rota da Página de Listagem de Disciplinas
app.get('/disciplinas', isAuthenticated, async function(req, res) {
    try {
        ...
        res.render('disciplinas', {
            ...
            username: req.session.username,
            notification: res.locals.notification,
            disciplines: allDisciplines.map(d => d.toJSON())
        });
    } catch (error) {
...
// Rota do Boletim (Protegida)
app.get('/boletim', isAuthenticated, async function(req, res) {
    try {
        const disciplines = await Disciplina.findAll({
            attributes: ['disccodigo', 'discnome'],
            order: [['discnome', 'ASC']]
        });
        ...
        res.render('boletim', {
            ...
            notification: res.locals.notification,
            currentPath: req.path,
            disciplinesJson: JSON.stringify(disciplines.map(d => d.toJSON())),
            userId: req.session.userId // <--- ADICIONADO: Passa o ID do usuário da sessão
        });
    } catch (error) {
        console.error('Erro ao carregar página do boletim:', error);
        req.session.notification = { type: 'error', title: 'Erro', message: 'Não foi possível carregar a página do boletim.' };
        res.redirect('/home');
    }
});

// MODIFICADO: GET /api/grades: Listar notas do usuário
app.get('/api/grades', isAuthenticated, async (req, res) => {
    const usucodigo = req.query.usucodigo || req.session.userId;

    if (!usucodigo) {
        return res.status(401).json({ message: 'Usuário não autenticado. usucodigo ausente.' });
    }

    const { semesterYear } = req.query; // Ex: "2025-1" ou "all"
    ...
    if (semesterYear && semesterYear !== 'all') {
        ...
        if (year && semester) {
            whereClause.boano = parseInt(year);
            whereClause.bosemestre = parseInt(semester);
        }
    }
    ...
    try {
        const grades = await Boletim.findAll({
            where: whereClause,
            include: [
                {
                    model: Disciplina,
            ...
            order: [['boano', 'DESC'], ['bosemestre', 'DESC'], [Disciplina, 'discnome', 'ASC']]
        });
        
        console.log('Notas encontradas:', grades.length); // Debug
...
// POST /api/grades: Adicionar uma nova nota
app.post('/api/grades', isAuthenticated, async (req, res) => {
    const { disccodigo, boano, bosemestre, n1, n2, n3 } = req.body;
    const usucodigo = req.session.userId;

    if (!usucodigo || !disccodigo || !boano || !bosemestre) {
        return res.status(400).json({ message: 'Campos obrigatórios ausentes para adicionar nota.' });
    }

    try {
        // Busca o nome da disciplina para salvar em 'bonome'
        const discipline = await Disciplina.findByPk(disccodigo);
        if (!discipline) {
            return res.status(404).json({ message: 'Disciplina não encontrada.' });
        }

        // Verifica se já existe uma entrada para evitar duplicatas com base no índice único
        const existingEntry = await Boletim.findOne({
            where: { usucodigo, disccodigo, boano, bosemestre }
        });

        if (existingEntry) {
            return res.status(409).json({ message: 'Já existe um boletim para esta disciplina, ano e semestre.' });
        }

        const newGrade = await Boletim.create({
            usucodigo: usucodigo,
            disccodigo: disccodigo,
            bonome: discipline.discnome, // Salva o nome da disciplina para acesso direto
            boano: boano,
            bosemestre: bosemestre,
            n1: n1 || null, // Garante null se não fornecido
            n2: n2 || null,
            n3: n3 || null
        });
        res.status(201).json({ message: 'Nota adicionada com sucesso!', grade: newGrade });
    } catch (error) {
...
// PUT /api/grades/:id: Atualizar uma nota existente
app.put('/api/grades/:id', isAuthenticated, async (req, res) => {
    const bocodigo = req.params.id;
    ...
    try {
        const grade = await Boletim.findOne({
            where: { bocodigo: bocodigo, usucodigo: usucodigo }
        });

        if (!grade) {
            return res.status(404).json({ message: 'Nota não encontrada ou não pertence a este usuário.' });
        }
        ...
        res.status(200).json({ message: 'Nota atualizada com sucesso!', grade: grade });
    } catch (error) {
        console.error('Erro ao atualizar nota:', error);
        res.status(500).json({ message: 'Erro interno ao atualizar nota.' });
    }
});

// DELETE /api/grades/:id: Excluir uma nota
app.delete('/api/grades/:id', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Erro interno ao excluir nota.' });
    }
});
